...
TSIL_COMPLEX TSIL_M0x0y0 (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S)
{
  ...
  if (TSIL_CABS(1.0L -S/X) < TSIL_TOL) return TSIL_M0x0y0Atx (X, Y);
  if (TSIL_CABS(1.0L -S/Y) < TSIL_TOL) return TSIL_M0x0y0Atx (Y, X);
...
/* ******************************************************************* */
/* New in v1.44, based on analytic limit of M0x0y0 with s -> x.        */

TSIL_COMPLEX TSIL_M0x0y0Atx (TSIL_REAL X, TSIL_REAL Y)
{
  TSIL_COMPLEX xoy, onemxoy, ln1mxoy, lnmx, a1, a2, lna1, lna2, result;
  TSIL_REAL lnx, lnxoy;

  /* None of the following if statements should ever be true... */
  if (X < TSIL_TOL) return TSIL_M000x0(Y,0);
  if (Y < TSIL_TOL) return TSIL_M000x0(X,X);
  if (TSIL_FABS(1.0L - X/Y) < TSIL_TOL) return TSIL_M0x0x0 (X, X);
  if (TSIL_FABS(X) < 10.0L*TSIL_TOL) {
    TSIL_Warn("TSIL_M0x0y0Atx", "M(0,x,0,y,0) is undefined at s = x = 0.");
    return TSIL_Infinity;
  }

  xoy = TSIL_AddIeps(X/Y);
  onemxoy = 1.L - xoy;
  ln1mxoy = TSIL_CLOG(onemxoy);
  lnxoy = TSIL_LOG(X/Y);
  lnx = TSIL_LOG(X);
  lnmx = lnx - I*PI;
  a1 = 0.5L + 0.5L * TSIL_CSQRT(1 - 4.L * xoy);
  a2 = a1 - onemxoy;
  lna1 = TSIL_CLOG(a1);
  lna2 = TSIL_CLOG(a2);

  result = (ln1mxoy*lna1*lna1 - 2.L*lna1*lna1*lna1 - 2.L*ln1mxoy*lna1*lna2 
    + lna1*lna1*lna2 + ln1mxoy*lna2*lna2 - lna1*lna2*lna2/2.L 
    - lna2*lna2*lna2/2.L + lna1*lna2*lnmx + 3.L*lna2*lna2*lnmx/2.L 
    - lna1*lna2*lnx - 3.L*lna2*lna2*lnx/2.L + ln1mxoy*lna1*lnxoy 
    + 3.L*lna1*lna1*lnxoy - ln1mxoy*lna2*lnxoy + 2.L*lna1*lna2*lnxoy 
    + 3.L*lna2*lna2*lnxoy/4.L - 2.L*lna1*lnmx*lnxoy - 7.L*lna2*lnmx*lnxoy/2.L 
    + 2.L*lna1*lnx*lnxoy + 7.L*lna2*lnx*lnxoy/2.L + ln1mxoy*lnxoy*lnxoy/2.L 
    - 2.L*lna1*lnxoy*lnxoy + 3.L*lnmx*lnxoy*lnxoy/2.L - 3.L*lnx*lnxoy*lnxoy/2.L 
    - 2.L*lnxoy*lnxoy*lnxoy/3.L - 5.L*lna1*Zeta2 + 6.L*lna2*Zeta2 
    - 13.L*lnxoy*Zeta2/2.L - 5*Zeta3 
    + (4.L*lna1 - 2.L*lnxoy)*TSIL_Dilog(xoy/a1) 
    + (2.L*ln1mxoy - lnxoy)* (-TSIL_Dilog(onemxoy) + TSIL_Dilog(onemxoy/a1) + 
      TSIL_Dilog(a1*onemxoy/xoy)) 
    + 3.L*TSIL_Trilog(a1) + 2.L*TSIL_Trilog(onemxoy) - TSIL_Trilog(onemxoy/a1) 
    + TSIL_Trilog(-onemxoy/a2) + TSIL_Trilog(xoy) + 3.L*TSIL_Trilog(xoy/a1) 
    - TSIL_Trilog(a1*onemxoy/xoy) + TSIL_Trilog(-a2*onemxoy/(xoy*xoy)) )/X;
